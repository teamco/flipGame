...
const EditableTags = props => {
  ...
  const handleClose = removedTag => {
    onChange(_tags.filter(tag => tag !== removedTag));
  };

  useEffect(() => {
    inputVisible && inputRef?.current?.focus();
  }, [inputVisible, inputRef?.current]);

  useEffect(() => {
    editInputIndex > -1 && editRef?.current?.focus();
  }, [editInputIndex, editRef?.current]);
